  ...
  return (
    <main className="app-shell">
      <section className="map-stage" aria-label="부산대학교 부산캠퍼스 고양이 위치 지도">
        ...
        <header className="floating-header"><div className="brand-pill"><span className="mini-cat" aria-hidden="true"><i /></span><div><b>meow map</b><small>{hasSupabaseConfig ? 'PNU CAT MAP' : 'LOCAL PREVIEW'}</small></div></div><button className="count-pill" type="button" onClick={() => setTab('cats')}><span>{cats.length}</span> 마리</button></header>
